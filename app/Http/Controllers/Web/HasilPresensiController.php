...
class HasilPresensiController extends Controller
{
    ...
    public function fetchRecordPresensi($kodePegawai)
    {
        $presensi = Presensi::leftJoin('rule_telat as telat1', 'telat1.id', '=', 'presensi.idRuleTelatMasuk')
            ->leftJoin('rule_telat as telat2', 'telat2.id', '=', 'presensi.idRuleLewatPulang')
            ...
            ->select('presensi.*', 'telat1.namaRuleTelat as statusTelatMasuk', 'telat2.namaRuleTelat as statusTelatPulang')
            ->get();
        return response()->json(["code" => 200, "data" => $presensi]);
    }

    public function cetakRuanganPresensi($kodeRuangan)
    {
    //    $presensi = Presensi::join('pegawai', 'pegawai.code', '=', 'presensi.pegawaiCode')
    //             ->where('pegawai.idDivisi', $kodeRuangan)
    //             ->select('pegawai.*')
    }
